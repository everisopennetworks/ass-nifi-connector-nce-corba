...
public abstract class NodeList_THelper
{
	...
	public static void insert (org.omg.CORBA.Any any, mtnm.tmforum.org.TopoManagementManager.Node_T[] s)
	{
		any.type (type ());
	...
	public static mtnm.tmforum.org.TopoManagementManager.Node_T[] read (final org.omg.CORBA.portable.InputStream _in)
	{
		mtnm.tmforum.org.TopoManagementManager.Node_T[] _result;
		int _l_result180 = _in.read_long();
		try
		{
			 int x = _in.available();
			 if ( x > 0 && _l_result180 > x )
				{
					throw new org.omg.CORBA.MARSHAL("Sequence length too large. Only " + x + " available and trying to assign " + _l_result180);
				}
		}
		catch (java.io.IOException e)
		{
		}
		_result = new mtnm.tmforum.org.TopoManagementManager.Node_T[_l_result180];
		for (int i=0;i<_result.length;i++)
		{
			_result[i]=mtnm.tmforum.org.TopoManagementManager.Node_THelper.read(_in);
		}

		return _result;
	}

	public static void write (final org.omg.CORBA.portable.OutputStream _out, mtnm.tmforum.org.TopoManagementManager.Node_T[] _s)
	{
		
		_out.write_long(_s.length);
		for (int i=0; i<_s.length;i++)
		{
			mtnm.tmforum.org.TopoManagementManager.Node_THelper.write(_out,_s[i]);
		}
