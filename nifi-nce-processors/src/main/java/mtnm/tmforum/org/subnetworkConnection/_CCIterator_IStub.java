...
public class _CCIterator_IStub
	extends org.omg.CORBA.portable.ObjectImpl
	implements mtnm.tmforum.org.subnetworkConnection.CCIterator_I
{
	/** Serial version UID. */
	private static final long serialVersionUID = 1L;
	private String[] ids = {"IDL:mtnm.tmforum.org/subnetworkConnection/CCIterator_I:1.0"};
	public String[] _ids()
	{
		return ids;
	}

	public final static java.lang.Class _opsClass = mtnm.tmforum.org.subnetworkConnection.CCIterator_IOperations.class;
	public int getLength() throws mtnm.tmforum.org.globaldefs.ProcessingFailureException
	{
		...
		else
		{
			...
			if( _so == null )
				continue;
			CCIterator_IOperations _localServant = (CCIterator_IOperations)_so.servant;
			int _result;
			try
			{
	...
	public boolean next_n(int how_many, mtnm.tmforum.org.subnetworkConnection.CrossConnectList_THolder ccList) throws mtnm.tmforum.org.globaldefs.ProcessingFailureException
	{
		while(true)
		{
			if(! this._is_local())
			{
				...
				try
				{
					...
					ccList.value = mtnm.tmforum.org.subnetworkConnection.CrossConnectList_THelper.read(_is);
					return _result;
				}
				catch( org.omg.CORBA.portable.RemarshalException _rx )
					{
		...
		else
		{
			...
			try
			{
				_result = _localServant.next_n(how_many,ccList);
				if ( _so instanceof org.omg.CORBA.portable.ServantObjectExt) 
					((org.omg.CORBA.portable.ServantObjectExt)_so).normalCompletion();
				return _result;
			}
